...
RUN apt-get update \ 
  && apt-get install -y --no-install-recommends apt-utils tzdata locales libssl-dev \
  apt-transport-https gnupg2 ca-certificates libfontconfig1-dev && echo "en_US.UTF-8 UTF-8" >> /etc/locale.gen \
...
RUN  echo "deb http://cloud.r-project.org/bin/linux/debian bullseye-cran40/" > /etc/apt/sources.list.d/cran.list \
  ...
  libproj-dev libxml2-dev libsqlite3-dev gdal-bin libudunits2-dev libfontconfig1-dev libcairo2-dev libcgal-dev \
