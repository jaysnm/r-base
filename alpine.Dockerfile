...
RUN apk add --no-cache --update-cache --virtual wget curl make libtool libpq autoconf \ 
  automake pkgconfig g++ libgcc file libc-dev tzdata linux-headers curl-dev imagemagick-dev \
...
RUN apk add --no-cache --update-cache --virtual harfbuzz-dev fribidi-dev libgit2-dev git cairo-dev \
  fontconfig-dev netcdf-dev libx11-dev graphicsmagick-dev sqlite-dev cargo libmagic libxml2-dev \
  gawk texlive freetype-dev texmf-dist gdal gdal-dev proj proj-dev geos-dev R R-dev R-doc \
